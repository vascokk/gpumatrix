#ifndef TVMET_XPR_COMPOUND_ASSIGNMENT_H
#define TVMET_XPR_COMPOUND_ASSIGNMENT_H
...
namespace gpumatrix
{
	...
	namespace gpu
	{
		...
		{
			array_kernel::scalar_array_compound_op(dest.data(),(typename Dest::value_type)alpha,dest.size(),fn);
		}
	...
	// Dest = Matrix
		...
		void do_compound_assign(Dest& dest, const Matrix<T> & m, const Func& fn)
		{
			...
			array_kernel::array_compound_op(dest.data(),m.data(),m.size(),fn);
		}
		// Dest = Matrix
		template <typename T,typename Dest,typename Func> 
