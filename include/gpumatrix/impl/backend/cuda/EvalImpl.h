#ifndef TVMET_XPR_EVAL_IMPL_H
#define TVMET_XPR_EVAL_IMPL_H

#include <gpumatrix/backend/cuda/cublas_template.h>
#include <gpumatrix/backend/cuda/array_kernel.h>
#include <gpumatrix/backend/cuda/matrix_kernel.h>
...
#pragma region algebra operation
			...
			void eval(Dest& dest, const XprMatrixTranspose<E> & trans, const Assign& assign_fn)
			{
				...
				transpose<typename E::value_type> (dest.data(),A.data(),A.rows(),A.cols());
			}
			...
			void eval(Dest& dest, const XprMMProduct<E1,E2> & prod, const Assign& assign_fn)
			{
				...
				cublas_gemm<value_type> ('N', 'N', A.rows(), B.cols(), A.cols(), 1, A.data(),
			...
			void eval(Dest& dest, const XprMtMProduct<E1,E2> & prod, const Assign& assign_fn)
			{
				...
				cublas_gemm<value_type> ('T', 'N', A.cols(), B.cols(), A.rows(), 1, A.data(),
					A.rows(), B.data(), B.rows(), 0, dest.data(), dest.rows());
			}
			...
			void eval(Dest& dest, const XprMMtProduct<E1,E2> & prod, const Assign& assign_fn)
			{
				...
				cublas_gemm<value_type> ('N', 'T', A.rows(), B.rows(), A.cols(), 1, A.data(),
					A.rows(), B.data(), B.rows(), 0, dest.data(), dest.rows());
			}
			...
			void eval(Dest& dest, const XprMtMtProduct<E1,E2> & prod, const Assign& assign_fn)
			{
				...
				cublas_gemm<value_type> ('T', 'T', A.cols(), B.rows(), A.rows(), 1, A.data(),
					A.rows(), B.data(), B.rows(), 0, dest.data(), dest.rows());
			}
			...
			void eval(Dest& dest, const XprMVProduct<E1,E2> & prod, const Assign& assign_fn)
			{
				...
				cublas_gemv <value_type>('N', A.rows(),A.cols(), 1, A.data(),  A.rows(), B.data(), 1, 0, dest.data(),1);
			...
			void eval(Dest& dest, const XprMtVProduct<E1,E2> & prod, const Assign& assign_fn)
			{
				...
				cublas_gemv <value_type>('T', A.rows(),A.cols(), 1, A.data(),  A.rows(), B.data(), 1, 0, dest.data(),1);
...
#pragma region arithmetic operation
				...
				const Assign& assign_fn)
			{
				...
				scalar_array_mul(dest.data(),1.0/alpha,B.data(),dest.size());
			}
				...
				const Assign& assign_fn)
			{
				...
				scalar_array_add(dest.data(),alpha,B.data(),dest.size());
			}
				...
				const Assign& assign_fn)
			{
				...
				scalar_array_sub(dest.data(),alpha,B.data(),dest.size());
			}
				...
				const Assign& assign_fn)
			{
				...
				scalar_array_div(dest.data(),alpha,B.data(),dest.size());
			}
...
#pragma endregion
				...
				const Assign& assign_fn)
			{
				...
				array_mul(dest.data(),A.data(),B.data(),dest.size());
				...
				const Assign& assign_fn)
			{
				...
				array_div(dest.data(),A.data(),B.data(),dest.size());
				...
				const Assign& assign_fn)
			{
				...
				colwise_sum(dest.data(),M.data(),M.rows(),M.cols());
